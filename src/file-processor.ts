...
import { detectPRComments, removePRComments, PRComment } from './comment-detector';
...
export class FileProcessor {
  ...
  async processFiles(): Promise<ProcessingResult> {
    ...
    try {
      ...
      const supportedFiles = changedFiles.filter(file => 
        this.isSupportedFile(file.filename) && file.status !== 'removed'
      );
   ...
   */
  private async processFile(file: { filename: string; status: string; sha: string }): Promise<PRFile> {
   ...
   */
  async updateFile(file: PRFile, commitMessage: string): Promise<void> {
   ...
   */
  async updateFiles(files: PRFile[], baseCommitMessage: string): Promise<void> {
    if (!files.length) {
